...
int changeName(char oldName[], char newName[])
{
    ...
    if(fileNameStr.compare("..") == 0) {
        perror("cannot rn the parent dir!\n");
        return -1;
    }
...
int creatDir(char dirName[])
{
    ...
    if (strlen(dirName) >= FILENAME_MAX_LENGTH) {
        perror("the length is out of bound!\n");
        return -1;
    }
...
int addDirUnit(dirTable* myDirTable, char fileName[], int type, int FCBBlockNum)
{
    ...
    for (int i = 0; i < myDirTable->dirUnitAmount; i ++ ) {
        string name(myDirTable->dirs[i].fileName);  // 将字符数组char[]转成string比较
        if(fileNameStr.compare(name) == 0) {
            perror("The same file already exists!\n");
            return -1; 
        }
    }
    ...
    dirUnit newDir;
    int len = myDirTable->dirUnitAmount;
    myDirTable->dirs[len - 1] = newDir; // 初始化并且赋值
    strcpy(myDirTable->dirs[len - 1].fileName, fileName);
    myDirTable->dirs[len - 1].type = type;
    myDirTable->dirs[len - 1].startBlock = FCBBlockNum;
    return 0; 
}
